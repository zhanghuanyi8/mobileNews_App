...
<!--      验证码-->
      ...
      >
        ...
        <template #button>
<!--          倒计时-->
          <van-count-down :time="100 *5" format="ss s" v-if="isCount" @finish="isCount=false"/>
<!--          发送验证码-->
          <van-button size="small" type="defalut" round
                      class="send-sms-btn"
                      native-type="button"
                      @click="sendCode"
          >发送验证码</van-button>
...
export default {
  ...
  data () {
    return {
      ...
      // 检验规则
      userForMater: {
        mobile: [{ required: true, message: '请输入手机号' }, { pattern: /^1[3|5|7|8]\d{9}$/, message: '手机格式错误' }],
